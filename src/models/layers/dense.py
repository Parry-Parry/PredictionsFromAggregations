...
def _dense_generator(noise_shape : int, latent_dim : int, out_shape : int, i : int):
    ...
    generator.add(tfkl.Dense(latent_dim // 2, activation='relu', input_shape=(noise_shape,), name='generator_in_{}'.format(i)))
    generator.add(tfkl.Dense(latent_dim, activation='relu', name='generator_latent_{}'.format(i)))
    generator.add(tfkl.Dense(out_shape, activation='relu', name='generator_out_{}'.format(i)))
